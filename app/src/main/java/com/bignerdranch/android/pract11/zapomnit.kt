                ...
                for(i in Books){
                    ...
                    else
                    {
                        Executors.newSingleThreadExecutor().execute {
                            correct = 1
                            db.knigaDAO().saveBook(KnigaTypes(Books[count1].idKniga, "${editText.text}", "${editText1.text}", "${editText2.text}"))
                            db.knigaDAO().saveZhanr(KnigaZhanr(Book[count1].id, Books[count1].idKniga, "${editText3.text}", Date()))
                            count++
                        }
                        break
                    }
